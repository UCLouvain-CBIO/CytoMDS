...
- changed package citation (and in README.md) to peer reviewed journal
  article

### CytoMDS 1.3.6

- [`ggplotMarginalDensities()`](https://uclouvain-cbio.github.io/CytoMDS/reference/ggplotMarginalDensities.md):
  corrected a bug when channels were specified as marker names.
...
- [`ggplotDistFeatureImportance()`](https://uclouvain-cbio.github.io/CytoMDS/reference/ggplotDistFeatureImportance.md)
  now can be used to create a stacked bar ggplot object, displaying
  feature importance in a distance matrix (extracted from a `DistSum`
  object)

### CytoMDS 1.3.2

- added `pointSize` argument to
...
- added `lineWidth` parameter in
  [`ggplotSampleMDSShepard()`](https://uclouvain-cbio.github.io/CytoMDS/reference/ggplotSampleMDSShepard.md)
- running `plotly::ggplotly()` on
  [`ggplotSampleMDSShepard()`](https://uclouvain-cbio.github.io/CytoMDS/reference/ggplotSampleMDSShepard.md)
  output now displays row and column number for each distance point.
...
- re-factored code portions to replace, as much as possible, for loops
  by [`apply()`](https://rdrr.io/r/base/apply.html) family of functions.
...
- removed `useBiocParallel` parameters from various stats functions (use
...
- in
  [`ggplotSampleMDS()`](https://uclouvain-cbio.github.io/CytoMDS/reference/ggplotSampleMDS.md)
  : add label layer after
  [`geom_point()`](https://ggplot2.tidyverse.org/reference/geom_point.html)
  (no more before)

### CytoMDS 0.99.8

- renamed `getChannelSummaryStats()` into
...
- added `displayPointLabels` argument to
  [`ggplotSampleMDS()`](https://uclouvain-cbio.github.io/CytoMDS/reference/ggplotSampleMDS.md)
- added `displayLegend` argument to
...
- refactored the pairwise distance calculation code, by pre-computing
  the unidimensional histograms and store them instead of recalculating
  them each time a distance between 2 samples is calculated. This
  improves CPU time and memory consumption.

### CytoMDS 0.99.6

- added `subset` argument in
  [`ggplotSampleMDS()`](https://uclouvain-cbio.github.io/CytoMDS/reference/ggplotSampleMDS.md)
  and `ggplotSampleMDSWrapBiplots`

### CytoMDS 0.99.5

- renamed `getPairwiseEMDDist()` into
  [`pairwiseEMDDist()`](https://uclouvain-cbio.github.io/CytoMDS/reference/pairwiseEMDDist.md)
- in
  [`pairwiseEMDDist()`](https://uclouvain-cbio.github.io/CytoMDS/reference/pairwiseEMDDist.md),
  added support for `BiocParallel`, and allowed for not loading the
  whole flowSet in memory at once.
...
- new version of computeMetricMDS() which automatically sets the number
  of dimensions to reach a target *pseudo R squared*
- added ggplotly() functionality for output MDS plots
- in
  [`ggplotSampleMDS()`](https://uclouvain-cbio.github.io/CytoMDS/reference/ggplotSampleMDS.md),
  added `flipXAxis`, `flipYAxis` to possibly ease low dimensional
  projection comparisons
- in
  [`ggplotSampleMDS()`](https://uclouvain-cbio.github.io/CytoMDS/reference/ggplotSampleMDS.md),
  added `displayArrowLabels` to discard the arrow labels in biplot. Also
  added `arrowThreshold`. Moved arrow labels toward the end of the
  arrows.
- in
  [`ggplotSampleMDS()`](https://uclouvain-cbio.github.io/CytoMDS/reference/ggplotSampleMDS.md)
  and
  [`ggplotSampleMDSShepard()`](https://uclouvain-cbio.github.io/CytoMDS/reference/ggplotSampleMDSShepard.md):
  added `displayPseudoRSq` parameter.
